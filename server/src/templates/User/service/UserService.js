...
const signup = async (req, res) => {
  try {
    ...
    // const human = await UserUtils.validateHuman(req.body?.token);
    // if (!human) {
    //   res.status(400).json({ error: "Please, you're not fooling us, bot." });
    //   return;
    // }
    let user = await UserUtils.findUserByEmail(email);
    if (!user) {
...
const login = async (req, res) => {
  try {
    ...
    //   return;
    // }
    const user = await UserUtils.findUser(email, password);
